...
if [ "${1:-}" = pr ] && [ "${2:-}" = list ]; then
  event pr-list
  if [ "${GH_PR_STATE:-none}" = exists ]; then
...
chmod 755 "$BIN/gh"
PATH="$BIN:$PATH"
export PATH GH_EVENTS="$EVENTS" GH_CALLS="$CALLS"
...
reset_fixture() {
  ...
  git --git-dir="$STAGING" update-ref refs/guard/github/heads/main "$STAGE_OLD"
...
for symbol in \
  ...
  guard_merge_gate_validate zmerge_reread_all_merge_gates \
  zmerge_reread_head_branch_main_gates zmerge_reread_branch_gate \
  zmerge_reread_pre_push_gates zmerge_guard_recovery_preflight; do
...
expect_true 'production Guard gate 已加载' \
  '[ "$(type -t guard_merge_gate_validate)" = function ]'
...
for mutation in human-merge transaction lease branch head main; do
  reset_fixture
  case "$mutation" in
  ...
  expect_true "R3 refresh 后 ${mutation} 改变阻断 retry" \
    '[ "$MUTATION_RC" -ne 0 ] && [ "$(file_count "$TDIR/deliver-count")" = 1 ] && [ "$(git --git-dir="$STAGING" rev-parse refs/heads/main)" = "$MAIN_LATEST" ]'
  case "$mutation" in
    human-merge)
...
reset_fixture
GH_PR_STATE=exists
run_capture BRANCH_OK BRANCH_OK_RC zmerge_reread_all_merge_gates stale-ok
expect_eq 'R3 full reader 正确 branch PASS' 0 "$BRANCH_OK_RC"
git -C "$WT" branch -f "$OTHER_BR" "$Z_HEAD"
git -C "$WT" checkout -q "$OTHER_BR"
run_capture BRANCH_BAD BRANCH_BAD_RC zmerge_reread_all_merge_gates stale-ok
...
for symbol in \
  ...
  contract_review_validate contract_stale contract_require_diff_in_scope \
  task_fetch_issue task_find_matching_pr task_pr_view_json task_pr_fields_ok \
  contract_pr_validate z_required_contexts z_pr_checks_ok guard_merge_gate_validate \
  zmerge_reread_all_merge_gates zmerge_reread_head_branch_main_gates \
  zmerge_reread_branch_gate zmerge_reread_pre_push_gates zmerge_guard_recovery_preflight; do
  if grep -Eq "^[[:space:]]*${symbol}[[:space:]]*\(\)" "$0"; then
